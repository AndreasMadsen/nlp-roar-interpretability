...
class XLNetSingleSequenceToClass(BaseSingleSequenceToClass):
    ...
    def __init__(self, cachedir, embedding, hidden_size=None, num_of_classes=2):
        ...
        self.config.mem_len = 0
        self.model = transformers.XLNetForSequenceClassification.from_pretrained(
            'xlnet-base-cased',
            cache_dir=f'{cachedir}/huggingface/transformers',
            config=self.config)
        self.embedding = self.model.base_model.word_embedding
    ...
    def forward(self, batch: SequenceBatch, embedding_scale: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        predict = self.model.forward(
            ...
            token_type_ids=batch.sentence_type,
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(
            optimizer_grouped_parameters, lr=2e-5, betas=(0.9, 0.98), eps=1e-6
        )
