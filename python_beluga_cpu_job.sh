...
if [ -z "${RUN_SEEDS}" ]; then
    python -u -X faulthandler "$HOME/workspace/nlproar/$1" "${@:2}" --use-gpu False --num-workers 1 --persistent-dir $SCRATCH/nlproar
else
    for seed in $(echo $RUN_SEEDS)
    do
        echo Running $seed
        python -u -X faulthandler "$HOME/workspace/nlproar/$1" "${@:2}" --seed "$seed" --use-gpu False --num-workers 1 --persistent-dir $SCRATCH/nlproar
    done
fi
