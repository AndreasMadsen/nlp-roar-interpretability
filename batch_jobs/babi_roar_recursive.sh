...
do
    ...
    do
        ...
        do
            ...
            for k in {1..10}
            do
                if [ ! -f $SCRATCH"/comp550/results/roar/babi-${type}_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-50.json" ]; then
                    echo babi-${type}_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-50
                    if last_jobid=$(
                        sbatch --time=${time[$type $importance_measure]} --mem=6G --parsable ${dependency} \
                            -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                            -J babi-${type}_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-50 $(job_script gpu) \
            ...
            for k in {10..90..10}
            do
                if [ ! -f $SCRATCH"/comp550/results/roar/babi-${type}_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-50.json" ]; then
                    echo babi-${type}_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-50
                    if last_jobid=$(
                        sbatch --time=${time[$type $importance_measure]} --mem=6G --parsable ${dependency} \
                            -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                            -J babi-${type}_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-50 $(job_script gpu) \
                            experiments/babi.py --recursive \
