...
#                 ["roberta random"]="0:??:0" ["roberta mutual-information"]="0:??:0"                                ["roberta gradient"]="0:10:0" ["roberta integrated-gradient"]="2:25:0" ["roberta times-input-gradient"]="0:10:0"
...
do
    ...
    do
        ...
        do
            ...
            if last_jobid=$(
                    submit_seeds "${time[$model_type $importance_measure]}" "$seeds" "roar/imdb_${model_type}_s-%s_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" \
                        --parsable $dependency \
                        $(job_script gpu) \
        ...
        do
            if last_jobid=$(
                    submit_seeds "${time[$model_type $importance_measure]}" "$seeds" "roar/imdb_${model_type}_s-%s_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" \
                        --parsable $dependency \
                        $(job_script gpu) \
                        experiments/imdb.py --recursive \
                        --model-type "$model_type" \
