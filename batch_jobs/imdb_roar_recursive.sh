...
do
    ...
    do
        ...
        for k in {1..10}
        do
            if [ ! -f $SCRATCH"/comp550/results/roar/imdb_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" ]; then
                echo imdb_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-${riemann_samples}
                if last_jobid=$(
                        sbatch --time=${time[$importance_measure]} --mem=6G --parsable ${dependency} \
                            -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                            -J imdb_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-${riemann_samples} $(job_script gpu) \
        ...
        for k in {10..90..10}
        do
            if [ ! -f $SCRATCH"/comp550/results/roar/imdb_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" ]; then
                echo imdb_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-${riemann_samples}
                if last_jobid=$(
                        sbatch --time=${time[$importance_measure]} --mem=6G --parsable ${dependency} \
                            -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                            -J imdb_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-${riemann_samples} $(job_script gpu) \
                            experiments/imdb.py --recursive \
