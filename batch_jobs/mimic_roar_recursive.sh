...
#                 ["roberta anemia random"]="0:??:0"   ["roberta anemia mutual-information"]="0:??:0"                                           ["roberta anemia gradient"]="0:05:0"   ["roberta anemia integrated-gradient"]="0:27:0"   ["roberta anemia times-input-gradient"]="0:04:0"
#                 ["roberta diabetes random"]="0:??:0" ["roberta diabetes mutual-information"]="0:??:0"                                         ["roberta diabetes gradient"]="0:09:0" ["roberta diabetes integrated-gradient"]="1:11:0" ["roberta diabetes times-input-gradient"]="0:07:0" )
        ...
        do
            ...
            do
                ...
                do
                    ...
                    if last_jobid=$(
                        submit_seeds "${time[$model_type $subset $importance_measure]}" "$seed" "roar/mimic-${subset::1}_${model_type}_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" \
                            --parsable $dependency \
                            $(job_script gpu) \
                ...
                do
                    if last_jobid=$(
                        submit_seeds "${time[$model_type $subset $importance_measure]}" "$seed" "roar/mimic-${subset::1}_${model_type}_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" \
                            --parsable $dependency \
                            $(job_script gpu) \
                            experiments/mimic.py --recursive \
                            --model-type "$model_type" \
