    ...
    do
        ...
        if precompute_jobid=$(
            sbatch --time=${pre_time[$importance_measure]} --mem=6G --parsable \
                ...
                -J "sst-pre_s-${seed}_m-${importance_measure::1}_r-0_rs-${riemann_samples}" $(job_script gpu) \
        ...
        for k in {1..10}
        do
            if [ ! -f $SCRATCH"/comp550/results/roar/sst_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-${riemann_samples}.json" ]; then
                echo sst_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-${riemann_samples}
                sbatch --time=${roar_time} --mem=6G --dependency=afterok:${precompute_jobid} \
                    -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                    -J sst_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-${riemann_samples} $(job_script gpu) \
        ...
        for k in {10..90..10}
        do
            if [ ! -f $SCRATCH"/comp550/results/roar/sst_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-${riemann_samples}.json" ]; then
                echo sst_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-${riemann_samples}
                sbatch --time=${roar_time} --mem=6G --dependency=afterok:${precompute_jobid} \
                    -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                    -J sst_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-${riemann_samples} $(job_script gpu) \
                    experiments/stanford_sentiment.py \
