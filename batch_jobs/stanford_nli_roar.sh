    ...
    do
        if precompute_jobid=$(
            sbatch --time=${pre_time[$importance_measure]} --mem=24G --parsable \
                ...
                -J "snli-pre_s-${seed}_m-${importance_measure::1}_r-0_rs-50" $(job_script gpu) \
        ...
        for k in {1..10}
        do
            if [ ! -f $SCRATCH"/comp550/results/roar/snli_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-50.json" ]; then
                echo snli_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-50
                sbatch --time=${roar_time} --mem=24G --dependency=afterok:${precompute_jobid} \
                    -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                    -J snli_s-${seed}_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-50 $(job_script gpu) \
        ...
        for k in {10..90..10}
        do
            if [ ! -f $SCRATCH"/comp550/results/roar/snli_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-50.json" ]; then
                echo snli_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-50
                sbatch --time=${roar_time} --mem=24G --dependency=afterok:${precompute_jobid} \
                    -o $SCRATCH"/comp550/logs/%x.%j.out" -e $SCRATCH"/comp550/logs/%x.%j.err" \
                    -J snli_s-${seed}_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-50 $(job_script gpu) \
                    experiments/stanford_nli.py \
