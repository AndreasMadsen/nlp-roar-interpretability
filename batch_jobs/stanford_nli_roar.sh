...
do
    ...
    do
        ...
        if precompute_jobid=$(
            submit_seeds ${pre_time[$importance_measure]} "$seed" "importance_measure/snli-pre_rnn_s-%s_m-${importance_measure::1}_rs-${riemann_samples}.csv.gz" \
                --mem=24G --parsable \
                $(job_script gpu) \
        ...
        for k in {1..10}
        do
            submit_seeds ${roar_time} "$seed" "roar/snli_rnn_s-%s_k-${k}_y-c_m-${importance_measure::1}_r-0_rs-${riemann_samples}.json" \
                --mem=24G $dependency \
                $(job_script gpu) \
        ...
        do
            if [ "$k" -le 90 ] || [ "$importance_measure" = "random" ]; then
                submit_seeds ${roar_time} "$seed" "roar/snli_rnn_s-%s_k-${k}_y-q_m-${importance_measure::1}_r-0_rs-${riemann_samples}.json" \
                    --mem=24G $dependency \
                    $(job_script gpu) \
                    experiments/stanford_nli.py \
