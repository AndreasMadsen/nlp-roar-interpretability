...
do
    ...
    do
        if last_jobid=$(
            submit_seeds "${time[$importance_measure]}" "$seeds" "roar/sst_rnn_s-%s_k-${k}_y-c_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" \
    ...
    do
        if last_jobid=$(
            submit_seeds "${time[$importance_measure]}" "$seeds" "roar/sst_rnn_s-%s_k-${k}_y-q_m-${importance_measure::1}_r-1_rs-${riemann_samples}.json" \
            --mem=6G --parsable $dependency \
            $(job_script gpu) \
            experiments/stanford_sentiment.py --recursive \
